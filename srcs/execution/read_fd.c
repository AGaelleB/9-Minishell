...
/*   Updated: 2023/10/30 10:32:16 by abonnefo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execve_fd(t_command *current, t_env *env)
{
	...
	// data.current->token_head = current->token_head;
	data.infile = 0;
	data.index = 0;
	init_execve(current, &(data.child_pids));
	// printf("J'AI %d PIPES\n", data.current->nb_pipes); // print
	while(data.current)
	{
		if (data.current->nb_pipes != 0)
		{
			while(data.current->token_head)
			{
				// printf("JE SUIS %d\n", data.current->token_head->type); // print
				if (data.current->token_head->type == TYPE_EOF)
				{
					printf("JE SUIS UN HEREDOC AVEC PIPE\n"); // print
					// return ;
				}
				data.current->token_head = data.current->token_head->next;
			}
			data.current->token_head = current->token_head;
		}
	...
	while (data.current)
	{
		printf("JE SUIS PAS HEREDOC PIPE\n"); // print
